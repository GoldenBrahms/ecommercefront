...
const Header = ({ history }) => (
        ...
        <div>
            ...
            {!isAuthenticated() &&(
                <>
                    <Link className="btn btn-primary" style={{height:'40px',position:'absolute', right:'130px'}} to="/signin">Connexion</Link>
                    <Link className="btn btn-primary" style={{position:'absolute', right:'20px'}} to="/signup">S'inscrire</Link>
                </>
            ...
            {isAuthenticated() &&(
                <span onClick={() => {signout(() =>{
                    history.push('/')
                })}} className="btn btn-primary" style={{position:'absolute', right:'20px', color:'white'}}>Deconnexion</span>
            )}
            </div>
        </div>
